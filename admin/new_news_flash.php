...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    $pdfRel = save_upload(
      'pdf',
      realpath(__DIR__ . '/../assets/news_flash') ?: __DIR__ . '/../assets/news_flash',
      ['pdf'],
      50 * 1024 * 1024
    );
    ...
    $sql = "
      INSERT INTO news_flash
        (label, title, published_at, author_note, pdf_file, banner_file, is_published, created_at)
      VALUES
    ...
    header('Location: ' . $adminBase . '/news_flash.php');
    exit;
  } catch (Throwable $e) {
    $error = $e->getMessage();
  }
...
<section class="hero-green">
  <div class="shell">
    <div class="hero-inner">
      <div>
        <h1 class="hero-title">Create a New News Flash</h1>
  ...
  <form class="ui-card mag-form" method="post" enctype="multipart/form-data">
    <div class="grid2 gap16">
      <div class="f">
        ...
        <input id="label" name="label" type="text" placeholder="e.g., October 2025" required>
      </div>
    ...
    <div class="form-actions">
      <a class="btn ghost" href="<?= $adminBase ?>/news_flash.php">← Back</a>
      <button class="btn primary" type="submit">Save News Flash</button>
    </div>
  </form>
</section>
